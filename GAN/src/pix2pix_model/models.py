...
def deconv_block_unet_lstm(x, x_0_2, x_1_2, x_2_2, x_3_2, f, h, w, batch_size, name, bn_mode, bn_axis, bn=True, dropout=False):
    ...
    #x = Concatenate(axis=bn_axis)([x, x_0_2])
...
def generator_unet_deconv_lstm(img_dim, bn_mode, batch_size, model_name="generator_unet_deconv"):
    ...
    unet_input = Input(shape=(4, img_dim[0], img_dim[1], img_dim[2]), name="unet_input")
    print(np.shape(unet_input))
    ...
    lambda_0 = Lambda(lambda x: x[:,0,:,:,:])(unet_input)
    lambda_1 = Lambda(lambda x: x[:,1,:,:,:])(unet_input)
    lambda_2 = Lambda(lambda x: x[:,2,:,:,:])(unet_input)
    lambda_3 = Lambda(lambda x: x[:,3,:,:,:])(unet_input)

    list_encoder_0 = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_0_1", padding="same")(lambda_0)]
    list_encoder_1 = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_1_1", padding="same")(lambda_1)]
    list_encoder_2 = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_2_1", padding="same")(lambda_2)]
    list_encoder_3 = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_3_1", padding="same")(lambda_3)]
    # update current "image" h and w
    h, w = h / 2, w / 2
    for i, f in enumerate(list_nb_filters[1:]):
        name0 = "unet_conv2D_0_%s" % (i + 2)
        conv0 = conv_block_unet(list_encoder_0[-1], f, name0, bn_mode, bn_axis)
        list_encoder_0.append(conv0)

        name1 = "unet_conv2D_1_%s" % (i + 2)
        conv1 = conv_block_unet(list_encoder_1[-1], f, name1, bn_mode, bn_axis)
        list_encoder_1.append(conv1)

        name2 = "unet_conv2D_2_%s" % (i + 2)
        conv2 = conv_block_unet(list_encoder_2[-1], f, name2, bn_mode, bn_axis)
        list_encoder_2.append(conv2)

        name3 = "unet_conv2D_3_%s" % (i + 2)
...
def generator_unet_deconv_lstm2(img_dim, bn_mode, batch_size, model_name="generator_unet_deconv"):
    ...
    list_nb_filters = [nb_filters * min(8, (2 ** i)) for i in range(nb_conv)]
    # Encoder
    lambda_layer = Lambda(lambda x: x[:,0,:,:,:])(unet_input)
    list_encoder_0 = [Conv2D(list_nb_filters[0], (3, 3),
                           strides=(2, 2), name="unet_conv2D_0_1", padding="same")(lambda_layer)]
    # update current "image" h and w
    h, w = h / 2, w / 2
    for i, f in enumerate(list_nb_filters[1:]):
        name = "unet_conv2D_0_%s" % (i + 2)
        conv = conv_block_unet(list_encoder_0[-1], f, name, bn_mode, bn_axis)
        list_encoder_0.append(conv)

        h, w = h / 2, w / 2

    # Combine in LSTM:
    #print(np.shape(unet_concat))
    #unet_reshape = Reshape((1, 512))(list_encoder_0[-1])
    #print(np.shape(unet_reshape))
    #unet_lstm = LSTM(512, input_shape=(1,512))(unet_reshape)
    #unet_reshape_back = Reshape((1, 1, 512))(unet_lstm)
    ...
    # Decoder
    list_decoder = [deconv_block_unet_lstm(list_encoder_0[-1], list_encoder_0[-2], list_encoder_0[-2],list_encoder_0[-2],list_encoder_0[-2],
                                      list_nb_filters[0], h, w, batch_size,
                                      "unet_upconv2D_1", bn_mode, bn_axis, dropout=True)]
    h, w = h * 2, w * 2
    for i, f in enumerate(list_nb_filters[1:]):
        ...
        if i < 2:
            ...
            d = False
        conv = deconv_block_unet_lstm(list_decoder[-1], list_encoder_0[-(i + 3)], list_encoder_0[-(i + 3)],list_encoder_0[-(i + 3)],list_encoder_0[-(i + 3)], f, h,
                                 w, batch_size, name, bn_mode, bn_axis, dropout=d)
        list_decoder.append(conv)
        h, w = h * 2, w * 2
...
def DCGAN(generator, discriminator_model, img_dim, patch_size, image_dim_ordering):
    ...
    gen_input = Input(shape=(4, img_dim[0], img_dim[1], img_dim[2]), name="DCGAN_input")
...
def load(model_name, img_dim, nb_patch, bn_mode, use_mbd, batch_size):
    ...
    if model_name == "generator_unet_deconv":
        model = generator_unet_deconv_lstm(img_dim, bn_mode, batch_size, model_name=model_name)
        model.summary()
        from keras.utils import plot_model
        plot_model(model, to_file="../../figures/%s.png" % model_name, show_shapes=True, show_layer_names=True)
        return model
